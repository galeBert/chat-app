  ...
  return (
    <table className="table-responsive w-full overflow-scroll h-4">
      ...
      <tbody className="relative">
        {!!data && !!data.length && data.map(({ adminName, adminRole, action, data: dataNotif, id }, idx) => (
          <tr key={idx}>
            <td className="text-center">
              <span>{adminName}</span>
            </td>
            <td className="text-center">{adminRole}</td>
            <td className="text-center flex justify-center items-center h-full">{action}</td>
            <td className="">
              <div className="flex row-username">
                <img
                  ...
                  src={noPhoto}
                />
                <Link to={`/user/user1`}><span>{dataNotif.username}</span></Link>
              </div>
            </td>
            <td className="text-center flex justify-center items-center h-full"><StatusContainer id={1} loading={false}>active</StatusContainer></td>
            <td className='row-action'>
              <div className="flex justify-center gap-6">
                <button className="w-4 h-4" onClick={handleApproveRequest(id, false)}><XIcon /></button>
                <button className="w-4 h-4" onClick={handleApproveRequest(id, true)}><CheckIcon /></button>
              </div>
            </td>
          </tr>
        ))}
