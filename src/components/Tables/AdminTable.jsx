...
import { SET_STATUS_ADMIN, SEARCH_ADMINS } from "graphql/mutation";
...
const AdminTable = ({ data, isLoading }) => {
  ...
  return (
    <table className="table-responsive w-full overflow-scroll h-4">
      ...
      <tbody>
        {data && !data.length ? (<h1>test</h1>) : (data && data.map(({ name, level, profileImage, email, id, isDeleted, isActive, isBanned }, idx) => {
          return (
            <tr>
              <td className="p-5">
              ...
              <td className="text-center flex justify-center items-center h-full">
                <StatusContainer active={isActive} loading={id === adminId && loading} id={name} >
                  {isBanned ? 'Banned' : (isActive ? 'Active' : (isDeleted && 'Deleted'))}
                </StatusContainer>
              </td>
              <td className='row-action'>
                <NewDropdown
                  uniqueId={idx}
                  options={[
                    { label: 'Active', onClick: () => handleChangeUserStatus(id, 'active') },
                    { label: 'Banned', onClick: () => handleChangeUserStatus(id, 'banned') },
                  ]}
                />
              </td>
            </tr>
