...
import { CHANGE_POST_STATUS, CHANGE_USER_STATUS, SEARCH_POST, SEARCH_USER } from "graphql/mutation";
import { useModal } from 'hooks/useModal';

export const usePostStatus = (shouldBeUpdateCache = true, currentPages, gqlSchema, postId, props) => {
  ...
  const [changePostStatus, { loading }] = useMutation(CHANGE_POST_STATUS, {
    update(cache, { data: newDataMutation }) {
      ...
      const { searchPosts } = cache.readQuery({
        query: SEARCH_POST,
        variables: { search: (parseQs?.search || roomId) || '', perPage: 20, page: 0, sortBy: "desc", filters: { owner: !roomId ? (username || '') : '' } }
      });
      ...
      cache.writeQuery({
        query: SEARCH_POST,
        variables: { search: parseQs?.search || '', perPage: 20, page: 0, sortBy: "desc", filters: { owner: !roomId ? (username || '') : '' } },
        data: {
          searchPosts: {
            ...searchPosts,
            hits: newHits
